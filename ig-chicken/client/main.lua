...
function CatchingChicken()
	...
	while startCatching do
		...
		if veh and (dist < 3.0) then
			...
			ClearPedTasks(ped)
			DeleteEntity(prop)
			TriggerServerEvent("ig-chicken:server:AddAliveChicken")
		end
	end
end
...
function processAliveChicken(position)
	...
	if Player then
		...
		if Player then
			...
			TriggerServerEvent("ig-chicken:server:SlaughteredChicken", position)
			end, function()
...
function packChicken(position)
	...
	if Player then
		QBCore.Functions.Progressbar("packing_chicken", Lang:t("process.packing"), Config.PackagedChicken[position]["processing_time"], false, true, {disableMovement = true,
				...
				TriggerServerEvent("ig-chicken:server:PackagedChicken", position)
...
function SellingChicken()
	...
	if ped then
		...
		ClearPedTasks(ped)
		DeleteEntity(prop)
		TriggerServerEvent("ig-chicken:server:SellingMoney")
	end
end
