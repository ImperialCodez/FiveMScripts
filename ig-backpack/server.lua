    ...
    for i = 1,#config.Bags,1 do
        QBCore.Functions.CreateUseableItem(config.Bags[i].Item, function(source,item)
            if not item.info.id then
                ...
                TriggerClientEvent('ig-backpack:client:OpenBag',source,newitem.info.id,i)
            else
            TriggerClientEvent('ig-backpack:client:OpenBag',source,item.info.id,i)
            end
        end)
    end
elseif config.InvType == 'ox' then
    for i = 1,#config.Bags,1 do
        QBCore.Functions.CreateUseableItem(config.Bags[i].Item, function(source,item)
            ...
            TriggerClientEvent('ig-backpack:client:OpenBag',source,item.metadata.id,i)
        end)
    end
end